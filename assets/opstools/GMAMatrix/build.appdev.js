...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/GMAMatrix
        ...
        async.series([
            ...
            function(next){
                ...
                AD.log('<green>building</green> opstools/GMAMatrix');

                AD.spawn.command({
                    command:'./cjs',
                    // options:[path.join('opstools', 'GMAMatrix', 'build.js')],
                    options:[path.join('steal', 'buildjs'), 'OpsPortal', path.join('opstools', 'GMAMatrix')],
shouldEcho:true,
                    // exitTrigger:'opstools/GMAMatrix/production.css'
                })
                .fail(function(err){
                    AD.log.error('<red>could not complete opstools/GMAMatrix build!</red>');
                    next(err);
                })
                .then(function(){
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'GMAMatrix', 'production.js'), tag:'packages/OpsPortal-GMAMatrix.js', replace:'OpsPortal/production.js'}
                ];
